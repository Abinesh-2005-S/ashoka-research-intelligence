        ...
        <div className="container mx-auto px-6 py-6">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              ...
              <h1 className="text-2xl font-bold text-foreground">{summary?.display_name}</h1>
              <p className="text-sm text-muted-foreground mt-1">Live analytics</p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-6 py-8">
        ...
        {(activeTab === "publications" || activeTab === "faculty") && (
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-white border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 shadow-sm"
            />
          </div>
        )}
        ...
        {activeTab === "publications" && (
          <div className="space-y-3">
            {/* Toolbar */}
            <div className="flex flex-col md:flex-row items-center justify-between mb-4 gap-4 bg-white p-3 rounded-lg border border-border shadow-sm">
              <div className="flex items-center gap-4 flex-wrap">
                <span className="text-sm font-bold text-foreground">{filteredPubs.length} publications {isLoadingPubs && "(Loading...)"}</span>
                <select value={filterYear} onChange={e => setFilterYear(e.target.value)} className="text-sm border border-border rounded p-1">
                  <option value="all">All Years</option>
                  {trendYears.map((t: any) => <option key={t.key} value={t.key}>{t.key}</option>).reverse()}
                </select>
                <select value={filterType} onChange={e => setFilterType(e.target.value)} className="text-sm border border-border rounded p-1">
                ...
                <label className="flex items-center gap-1 text-sm text-foreground cursor-pointer">
                ...
                <div className="flex gap-1 border-r border-border pr-3">
                  {[["citations", "Most Cited"], ["newest", "Newest First"]].map(([val, label]) => (
                    <button key={val} onClick={() => setSortBy(val as any)}
                      className={`px-3 py-1.5 rounded text-xs font-semibold border transition-all ${sortBy === val ? "bg-blue-600 text-white border-blue-600" : "bg-secondary text-muted-foreground border-border hover:bg-secondary"
                        }`}>{label}</button>
                  ))}
                </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground mb-2 flex flex-wrap items-center gap-1">
                      ...
                      {pub.authorships?.length > 3 && (
                        <div className="group relative inline-block">
                          <span className="text-blue-700 bg-blue-50 px-1 py-0.5 rounded cursor-pointer ml-1 whitespace-nowrap hover:bg-blue-100 transition-colors">
                          ...
                          <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 hidden group-hover:block w-64 p-3 bg-slate-800 text-white text-xs rounded-lg shadow-xl z-20">
                            <div className="mb-1 text-muted-foreground font-semibold uppercase tracking-wider text-[10px]">Additional Authors</div>
                    ...
                    {pub.primary_location?.source?.display_name && (
                      <p className="text-xs text-muted-foreground mt-0.5 italic">{pub.primary_location.source.display_name}</p>
                    )}
                    ...
                    {pub.sustainable_development_goals && pub.sustainable_development_goals.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {pub.sustainable_development_goals.slice(0, 3).map((sdg: any) => {
                          ...
                          return (
                            <div key={sdg.id} className="flex items-center gap-1.5 pr-2 bg-secondary text-foreground rounded border border-border text-[10px] font-semibold tracking-wide overflow-hidden shadow-sm hover:shadow transition-shadow cursor-default" title={sdg.display_name}>
                              <img
                  ...
                  <div className="text-right shrink-0 min-w-[80px]">
                    <div className="text-xl font-bold text-foreground">{pub.cited_by_count ?? 0}</div>
                    <div className="text-xs text-muted-foreground mb-2">citations</div>
                    {pub.doi && (
            ...
            {/* Pagination Controls */}
            {totalPubs > 0 && (
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-border text-sm">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 text-muted-foreground hover:text-foreground disabled:opacity-40 disabled:hover:text-muted-foreground cursor-pointer"
                  >
              ...
              {filteredAuthors.map((author: any, i) => (
                <div key={author.id} className="pro-card p-5 hover:shadow-md transition-shadow flex flex-col">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-foreground font-bold shrink-0">
                      {author.display_name?.charAt(0)}
                    </div>
                    <div>
                      <div className="font-semibold text-foreground text-sm leading-snug">{author.display_name}</div>
                      <div className="text-xs text-muted-foreground mt-0.5">{author.last_known_institutions?.[0]?.display_name || "Ashoka University"}</div>
                    </div>
                  </div>
        ...
        {activeTab === "disciplines" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="pro-card p-6">
              <h2 className="font-bold text-foreground mb-1">Research Areas</h2>
              <p className="text-sm text-muted-foreground mb-6">Ranked by publication count</p>
              <div className="space-y-4">
                {topTopics.map((t: any) => (
                  <div key={t.id}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-sm text-foreground font-medium truncate pr-2">{t.display_name}</span>
                      <span className="text-sm font-bold text-foreground shrink-0">{t.count.toLocaleString("en-US")}</span>
                    </div>
            ...
            {/* Article Types */}
            <div className="pro-card p-6">
              <h2 className="font-bold text-foreground mb-1">Article Type Distribution</h2>
              <p className="text-sm text-muted-foreground mb-6">Publication output by research type</p>
              <div className="space-y-3">
                {topTypes.map((t: any) => (
                  <div key={t.key}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-sm text-foreground font-medium capitalize">{(t.key_display_name ?? t.key).replace(/-/g, " ")}</span>
                      <span className="text-sm font-bold text-foreground">{t.count.toLocaleString("en-US")}</span>
                    </div>
                    <div className="data-bar-track">
            ...
            <div className="pro-card p-6 lg:col-span-2">
              <h2 className="font-bold text-foreground mb-1">Publication Velocity</h2>
              <p className="text-sm text-muted-foreground mb-6">Annual research output, 2015 to present</p>
        ...
        {activeTab === "openscience" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* OA Breakdown */}
            <div className="pro-card p-6">
              <h2 className="font-bold text-foreground mb-1">Open Access Breakdown</h2>
              <p className="text-sm text-muted-foreground mb-6">Distribution of access status across all publications</p>
              ...
              <div className="space-y-3">
                {topOATypes.map((o: any, i: number) => {
                  const colors = ['bg-blue-500', 'bg-blue-400', 'bg-blue-300', 'bg-slate-300', 'bg-border'];
                  return (
                    <div key={o.key} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`w-2.5 h-2.5 rounded-full ${colors[i] || 'bg-border'} shrink-0`} />
                        <span className="text-sm text-foreground">{OA_LABEL[o.key] ?? o.key_display_name}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-sm font-bold text-foreground">{o.count.toLocaleString("en-US")}</span>
                        <span className="text-xs text-muted-foreground ml-1">({Math.round((o.count / totalWorks) * 100)}%)</span>
                      </div>
                    </div>
                  );
                })}
            ...
            {/* OA Over Time */}
            <div className="pro-card p-6">
              <h2 className="font-bold text-foreground mb-1">Open Access Over Time</h2>
              <p className="text-sm text-muted-foreground mb-6">OA share per year — growth trajectory</p>
              <div className="space-y-3">
                  ...
                  return (
                    <div key={t.key}>
                      <div className="flex justify-between text-xs mb-1.5">
                        ...
                        <span className="text-muted-foreground">{oaWorksCount} OA / {totalWorksCount} total · {oaPct}%</span>
                      </div>
                      <div className="data-bar-track">
            ...
            <div className="lg:col-span-2 bg-blue-50 border border-blue-100 rounded-xl p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-foreground mb-1">Open Science Commitment</h3>
        ...
        {activeTab === "collaborations" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 pro-card p-6">
              <h2 className="font-bold text-foreground mb-1">International Collaboration Network</h2>
              <p className="text-sm text-muted-foreground mb-6">Top partner countries by co-authored publications (excluding India)</p>
              <div className="space-y-4">
                  ...
                  <div key={c.key} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1.5">
                        <span className="text-sm font-medium text-foreground">{c.key_display_name}</span>
                        <span className="text-sm font-bold text-foreground">{c.count.toLocaleString("en-US")}</span>
                      </div>
                      <div className="data-bar-track">
            ...
            <div className="space-y-4">
              <div className="pro-card p-6">
                ...
                <p className="text-xs text-muted-foreground mt-2">Active international research collaborations</p>
              </div>
              <div className="pro-card p-5">
                <h3 className="text-sm font-bold text-foreground mb-3">Top Partners</h3>
                {topCountries.slice(0, 5).map((c: any, i) => (
                  <div key={c.key} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                    <span className="text-xs text-muted-foreground w-4">#{i + 1}</span>
                    <span>{COUNTRY_FLAGS[c.key] ?? "🌍"}</span>
                    <span className="text-sm text-foreground flex-1">{c.key_display_name}</span>
        ...
        {activeTab === "preprints" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="pro-card p-6 border-l-4 border-slate-300 bg-secondary/50">
              <h4 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Frontier Momentum Narrative</h4>
              <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="pro-card p-6">
                <h3 className="font-bold text-foreground mb-1">Preprint Velocity</h3>
                <p className="text-xs text-muted-foreground mb-6">Rolling preprint volume over recent years</p>
                <div className="h-64">
                  {isLoadingPreprints ? <div className="text-sm text-muted-foreground">Loading...</div> : (
              ...
              <div className="pro-card p-6">
                <h3 className="font-bold text-foreground mb-1">First-to-Market Timeline</h3>
                <p className="text-xs text-muted-foreground mb-6">Most recent preprint deposits</p>
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
                  {isLoadingPreprints ? <p className="text-sm text-muted-foreground">Loading...</p> : preprints.slice(0, 15).map((p: any) => (
                    <div key={p.id} className="border-l-2 border-border pl-4 py-1">
                      <div className="text-xs font-bold text-muted-foreground mb-1">{p.publication_year}</div>
                      <h4 className="text-sm font-semibold text-foreground leading-snug line-clamp-2">{p.title || "Untitled"}</h4>
                      {p.doi && <a href={p.doi.startsWith('http') ? p.doi : `https://doi.org/${p.doi}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-700 hover:underline mt-1 inline-block">View DOI</a>}
                    </div>
                  ))}
                  {preprints.length === 0 && !isLoadingPreprints && <p className="text-sm text-muted-foreground">No preprints found.</p>}
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === "taxonomy" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="pro-card p-6 border-l-4 border-slate-300 bg-secondary/50">
              <h4 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Intellectual Taxonomy</h4>
              <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            <div className="pro-card p-6 h-[700px] flex flex-col">
              <h3 className="font-bold text-foreground mb-1">Research Domain Hierarchy (Radial Map)</h3>
              <p className="text-xs text-muted-foreground mb-6">Interactive Sunburst map of domains (inner circle) and specialized fields (outer circle)</p>
              <div className="w-full h-[550px]">
                <ResponsiveContainer width="100%" height={550} minWidth={0}>
                  <PieChart>
                    <Tooltip
        ...
        {activeTab === "integrity" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="pro-card p-6 border-l-4 border-slate-300 bg-secondary/50">
              <h4 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Research Transparency</h4>
              <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            <div className="pro-card p-6">
              <h3 className="font-bold text-foreground mb-6">Retracted Publications Ledger</h3>
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                {isLoadingRetractions ? <p className="text-sm text-muted-foreground">Loading ledger...</p> : retractions.map((p: any) => (
                  <div key={p.id} className="border border-border rounded-lg p-4 bg-white shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                      <AlertTriangle className="w-4 h-4 text-muted-foreground" />
                      <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Retracted Work</span>
                      <span className="text-xs text-muted-foreground ml-auto">{p.publication_year}</span>
                    </div>
                    <h4 className="text-sm font-semibold text-foreground mb-1">{p.title || "Untitled"}</h4>
                    <p className="text-xs text-muted-foreground mb-2">
                      {p.authorships?.map((a: any) => a.author.display_name).join(", ")}
                    </p>
                    {p.doi && <a href={p.doi.startsWith('http') ? p.doi : `https://doi.org/${p.doi}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-700 hover:underline">View Record</a>}
                  </div>
                ))}
                {retractions.length === 0 && !isLoadingRetractions && <p className="text-sm text-muted-foreground">No retracted publications found in the ledger.</p>}
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "pedigree" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="pro-card p-6 border-l-4 border-slate-300 bg-secondary/50">
              <h4 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Global Talent Pedigree</h4>
              <p className="text-sm text-muted-foreground leading-relaxed">
              ...
              <div className="flex min-w-max items-start pl-4">
                ...
                <div className="flex items-center shrink-0 pt-[100px]">
                  <div className="px-6 py-4 bg-slate-800 text-white font-bold rounded-xl shadow-md whitespace-nowrap z-10 relative border border-slate-700">
                    <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Current Base</div>
                  ...
                  {authors.slice(0, 15).map((author: any) => {
                    ...
                    return (
                      <div key={author.id} className="relative flex items-center">
                        ...
                        {/* Author Node */}
                        <div className="shrink-0 px-5 py-3 bg-white border border-border rounded-lg shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] flex items-center gap-3 w-[260px] relative z-10 hover:border-slate-300 transition-colors">
                          <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-foreground font-bold shrink-0">
                            {author.display_name?.charAt(0)}
                          </div>
                          <div className="truncate">
                            <div className="font-semibold text-foreground text-sm truncate">{author.display_name}</div>
                            <div className="text-xs text-muted-foreground font-medium">Faculty/Scholar</div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-4 relative py-2 pl-8 border-l border-slate-300">
                          {otherInsts.map((inst: any, idx: number) => (
                            <div key={idx} className="relative flex items-center">
                              ...
                              <div className="absolute left-0 w-8 h-px bg-slate-300 -ml-8"></div>
                              <div className="px-4 py-2.5 bg-secondary hover:bg-secondary transition-colors border border-border rounded-md shadow-sm w-[320px] flex items-center gap-2 relative z-10">
