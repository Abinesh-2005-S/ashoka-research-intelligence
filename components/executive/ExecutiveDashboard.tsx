...
export function ExecutiveDashboard({ summary, authors, trends, oaBreakdown, countryCollabs }: ExecutiveDashboardProps) {
  ...
  return (
    <div className="bg-secondary w-full animate-in fade-in duration-500">
        ...
        <div className="container mx-auto px-6 py-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl font-bold text-foreground">{summary?.display_name}</h1>
              <div className="flex items-center gap-2 mt-2">
                <CheckCircle2 className="w-4 h-4 text-muted-foreground" />
                <span className="text-sm text-muted-foreground">Live data — verified</span>
              </div>
            </div>
            {/* Headline metrics */}
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div key={kpi.label} className="bg-secondary border border-border rounded-xl px-5 py-3 text-center min-w-[100px]">
      ...
      <div className="container mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="pro-card p-6">
            <h2 className="font-bold text-foreground mb-1">Research Output Velocity</h2>
            <p className="text-sm text-muted-foreground mb-6">Publications per year — strategic growth trajectory</p>
            <div className="flex items-end gap-2 h-36">
              {trendsSorted.map((t: any) => (
                <div key={t.key} className="flex-1 flex flex-col items-center gap-1 group">
                  <div className="text-xs font-semibold text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">{t.count}</div>
            ...
            <div className="mt-4 pt-4 border-t border-border flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">
                <span className="font-semibold text-foreground">+{growthPct}%</span> growth in {recentYear?.key} vs {prevYear?.key}
              </span>
            </div>
          </div>
          ...
          {/* Research Domains */}
          <div className="pro-card p-6">
            <h2 className="font-bold text-foreground mb-1">Strategic Research Domains</h2>
            <p className="text-sm text-muted-foreground mb-6">Top research areas by publication volume</p>
            <div className="space-y-4">
              {topTopics.map((t: any) => (
                <div key={t.id}>
                  <div className="flex justify-between items-start mb-1.5">
                    <span className="text-sm text-foreground font-medium pr-2 leading-snug">{t.display_name}</span>
        ...
        <div className="pro-card p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-bold text-foreground">Researcher Leaderboard</h2>
              <p className="text-sm text-muted-foreground mt-0.5">Top researchers by total citation impact</p>
            </div>
              ...
              <thead>
                <tr className="border-b border-border">
                  {["Rank", "Researcher", "Works", "Citations", "h-index"].map(h => (
                    <th key={h} className={`py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide ${h === "Rank" || h === "Researcher" ? "text-left px-3" : "text-right px-3"}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {authors.slice(0, 10).map((author: any, i) => (
                  <tr key={author.id} className="border-b border-border hover:bg-secondary transition-colors">
                    <td className="py-3 px-3 text-sm font-semibold text-muted-foreground">#{i + 1}</td>
                    ...
                    <td className="py-3 px-3 text-right font-semibold text-foreground">{author.works_count ?? "–"}</td>
                    <td className="py-3 px-3 text-right font-bold text-foreground">{(author.cited_by_count ?? 0).toLocaleString("en-US")}</td>
        ...
        {/* ── SDG ALIGNMENT ── */}
        <div className="pro-card p-6">
          <h2 className="font-bold text-foreground mb-1">UN SDG Research Alignment</h2>
          ...
          <div className="mt-4 flex items-center gap-6 text-xs text-muted-foreground">
            <div className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded bg-border border border-slate-300 inline-block" />
